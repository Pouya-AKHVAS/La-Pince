...
import { createTransactionSchema, updateTransactionSchema, transactionQuerySchema } from "../validators/transaction.validator.js"; // Import des schémas de validation Zod pour les transactions. Ces schémas permettent de valider les données entrantes (body, query params) de manière structurée et de fournir des messages d'erreur clairs en cas de données invalides.
...
export const getAll = async (req: Request, res: Response) => {
  try { 
    ...
    const transactions = await transactionService.getAllTransactions(req.user.id, filters.data);
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const transaction = await transactionService.createTransaction(req.user.id, body.data);
    
    return res.status(201).json(transaction);
  } catch (error: any) {
    // Si le service lève une erreur (ex: "Catégorie introuvable") on l'attrape ici
    if (error.message === "Catégorie introuvable" || error.message === "Action non autorisée sur cette catégorie") {
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (!body.success) return res.status(400).json({ erreurs: body.error.format() });

    // Vérification de sécurité passive (existe ET m'appartient)
    const existingTransaction = await transactionService.getTransactionById(id, req.user.id);
    if (!existingTransaction) return res.status(404).json({ message: "Transaction introuvable ou non autorisée" });

    const updatedTransaction = await transactionService.updateTransaction(id, req.user.id, body.data);
